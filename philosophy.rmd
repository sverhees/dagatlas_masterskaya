...
For example, the Andi language is spoken in 17 villages. There are 9 main villages which each have their own village idiom. These can be divided into two groups: Upper Andi and Lower Andi. In addition, there are 8 villages for which we have no information on the variety spoken there.
...
<font color = "dimgray">Dargwa is considered a single language with a number of highly divergent dialects by some, and a group of distinct but related languages by others. 

Since the atlas is based on descriptive literature, 

has long been considered a single language with a number of highly divergent dialects. The current consensus among linguists is to classify some of these former dialects as distinct languages belonging to a single Dargwa branch. In the atlas we include several Dargwa varieties for which documentation is available in our standard sample.</font color>   
