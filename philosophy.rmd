...
For example, the Andi language is spoken in 17 villages. 8 of these have no dialect classification. The remaining 9 villages each have a specific village idiom, which are grouped into Upper Andi and Lower Andi.

Let's say that for a straight-forward feature like **Number of noun classes**, we have general data on both the Upper and Lower dialects, and more accurate information on several villages from the Upper dialect group. One of these villages (Rikvani) even has a value that differs from the other varieties we have data for.
...
1. **Language and feature** shows the basic TALD visualization, where the inner dot is colored according to language, and the color of the outer dot shows the value for the linguistic feature.
2. **Feature and granularity** shows the distribution of the feature and the accuracy (=granularity) of the information for each datapoint.
3. **Toggle granularity** allows the user to show only certain levels of accuracy.

<span style="color:Firebrick">
В итоге, наверно, нужны только 1 и 3, если 3 бы сработала как надо. Хотим ли сохранить версию карт, на которой только одно наблодение - один язык - одна карта?
</span>
...
map.feature(alldata3$lang.x,
            ...
            label = alldata3$village,
            stroke.features = alldata3$granularity,
            stroke.color = c("white", "grey", "black"),
            stroke.title = "Data granularity",
            popup = paste("<b>Village:</b>", alldata3$village, "<br>",
                          "<b>Data:</b>", alldata3$granularity, "<br>",
                          "<b>Value:</b>", alldata3$value),
            zoom.control = T)
...
## 3. Toggle granularity

```{r, echo=FALSE, message=FALSE}

# То есть, тут нужно, чтобы карта показаывала только features = value, 
# но чтобы убиралка работала по столбцу granularity, и у меня не получается!

map.feature(alldata3$lang.x,
            latitude = alldata3$lat,
            longitude = alldata3$lon,
            features = alldata3$granularity,
            color = c("grey", "white", "black"),
            label = alldata3$village,
            stroke.features = alldata3$value,
            stroke.color = c("MistyRose", "Firebrick", "Thistle"),
            stroke.title = alldata3$feature[1],
            control = T,
            popup = paste("<b>Village:</b>", alldata3$village, "<br>",
...
Our coverage is far from sufficient, because we do not have data for many dialects, and we don't know the dialect affiliation of a large number of villages in the area.
...
Visualizations are created by combining datasets on linguistic features with [The East Caucasian villages dataset](https://github.com/sverhees/master_villages): a dataset that contains a list of villages in the eastern Caucasus, their coordinates, and the language spoken there.

The atlas uses its own version of the dataset, with some columns lacking and others added. Get the latest version of the original dataset using the link above. 

<span style="color:Firebrick">
Это нужно потому что для мердж датасэтов мне пришлось добавить в villages столбец default_level, где дублируется язык для тех сел, у которых нет диалектной принадлежности. Это чисто технический столбец, который не нужен другим пользователям датасэта с селами.
</span>

# Northeastern Daghestan

The northeastern part of Daghestan was settled only relatively recently, and is much more ethnically mixed than the rest of the area, as you can see on [these maps](https://sverhees.github.io/master_villages/sample_maps.html). In addition, little to nothing is known about the variants of the languages spoken there. In some cases we know the village of origin for most of its inhabitants (e.g. Novogagatli was founded by settlers from the Andi village Gagatli), but we don't know how well their dialect is preserved, and to what extent their population is ethnically and linguistically homogeneous. Therefore, we decided to exclude this region from our visualizations.
