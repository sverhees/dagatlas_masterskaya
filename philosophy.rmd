...
For example, the Andi language is spoken in 17 villages. 8 of them have no dialect classification. The remaining 9 villages each have their own village idiom, which can be divided into two groups: Upper Andi and Lower Andi.
...
The table below summarizes the different values observed for the language (the number of noun classes is indicated between brackets and each value is color-coded).
...
2. **Data granularity** allows the user to show only certain levels of data accuracy for the feature (e.g. uncheck "language" in the image below to remove all the unclassified Andi villages from the visualization).
...
Villages located in the northeastern part of Daghestan are not displayed in the Atlas.

This area was settled only relatively recently, and is much more ethnically mixed than the rest of the area, as you can see [here](https://sverhees.github.io/master_villages/maps_new.html). 

In addition, little to nothing is known about the varieties of the languages spoken there. In some cases we know the village of origin for most of its inhabitants (e.g. Novogagatli was founded by settlers from the Andi village Gagatli), but we do not know how well their dialect is preserved and to what extent their population is ethnically and linguistically homogeneous. Therefore, we decided to exclude this region from the visualizations in the atlas for now.

---

## List of languages

Below is a list of the languages included in our sample, grouped by language family and branch in alphabetical order
...
<font color = "dimgray">Since Dargwa subsumes a large number of highly divergent idioms, we included several Dargwa varieties for which documentation is available in our standard sample (in addition to literary Dargwa).</font color>  
...
<font color = "dimgray">Two mutually unintelligible varieties of Tat are spoken in the area: Judeo-Tat and Muslim Tat. 

Our sample contains only **Judeo-Tat**, which is spoken in the city of Derbent. 

Within Daghestan, Muslim Tat was spoken in the villages Dzhalgan and Mitagi. It is unclear to what extent the language is preserved there: the villages are labeled as Azerbaijani-speaking in our data, which could be due to erroneous self-identification or shift. 

Tat-speaking communities in Azerbaijan are currently not part of the sample because we lack their coordinates.</font color>

**Kartvelian**

* Georgian

Georgian is spoken in the Qakh district of Azerbaijan, alongside Tsakhur and Azerbaijani. It is also an important neighboring language.
